...
st.markdown("""
    ...
    p, label, .stMarkdown, .stSubheader { color: #ffffff !important; font-weight: 500; }
    ...
    .reset-btn { margin-top: 20px; border: 1px solid #ef4444 !important; color: #ef4444 !important; }
...
# --- 2. THE RESET BUTTON (PM REQUIREMENT) ---
def reset_app():
    ...
    if st.button("🚀 Synthesize Visualization"):
        with st.status("Agent is synthesizing...", expanded=True):
            try:
                ...
                # Principal PM Tip: Use a specific prompt that minimizes code errors
                sys_prompt = f"""
                ...
                if st.button("🔄 Emergency Reset Engine"):
                    reset_app()

    # Permanent Reset Button in Sidebar for better UX
    with st.sidebar:
        st.markdown("### 🛠 Tools")
