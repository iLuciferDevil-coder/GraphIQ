...
st.markdown("""
    ...
    button[kind="secondary"] { color: #39FF14 !important; border-color: #39FF14 !important; background-color: rgba(57, 255, 20, 0.05) !important; }
    .stFileUploader section { background-color: #111111 !important; border: 1px dashed #39FF14 !important; color: #ffffff !important; }
    .hero-box { padding: 50px 20px; text-align: center; background: radial-gradient(circle at center, rgba(57, 255, 20, 0.1) 0%, rgba(5, 5, 5, 0) 75%); margin-bottom: 20px; }
    ...
    .sid-pill { background: rgba(0, 0, 0, 0.95); padding: 12px 24px; border-radius: 50px; border: 1px solid #39FF14; box-shadow: 0 0 20px rgba(57, 255, 20, 0.4); display: flex; align-items: center; gap: 12px; }
...
if not file:
    ...
    try:
        ...
        with st.expander("🔍 View Raw Data Preview"): st.dataframe(df.head(5), use_container_width=True)
            ...
            if st.button("🚀 Synthesize Visualization", use_container_width=True):
                with st.status("Engine: Llama 3.3 Versatile Processing...", expanded=True):
                    try:
                        ...
                        # THE FINAL FIX: Using CodeInterpreter specifically
        ...
        with col_reset:
            if st.button("🗑 Reset", use_container_width=True): reset_app()
